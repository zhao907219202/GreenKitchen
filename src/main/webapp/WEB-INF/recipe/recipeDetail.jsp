...
<head>
<title>绿色厨房</title>
	<link rel="stylesheet" type="text/css"	href="../../css/recipe/recipeDetail.css" />
	<link rel="stylesheet" type="text/css" href="../../css/public/common.css" />
	<script type='text/javascript' language="javascript">
	var clicked = false;
	function submitComment(){
		if(clicked==false){
			document.comment.action="/reply/submitComment.action";
		}
		document.comment.submit();
	}
	
	function focusComment(obj){
		document.comment.action="/reply/submitCommentReply.action?comment_id="+obj.id;
	...
	<script type="text/javascript" src="/js/ajax/ajax_collect.js" ></script>
			...
			<div id="data" class="font3">
				<span style="font-size: 28px;color:orange">${recipe.commentednum } </span>　人评论 　　
				 ...
				 <span style="float:right;margin-right: 20px">
				 	...
				 	<input id="doCollect" type="image" src="../../images/recipe/collectbutton.jpg"  onclick="doCollect()"/>
				 	<input id="cancelCollect" type="image" src="../../images/recipe/cancelcollect.jpg"  onclick="cancelCollect()"/>
				 </span>
			</div>
			<div id="content">
				<a href="/info/goUserIndex.action?user_id=${recipe.user.id}"><span class="font6">${recipe.user.username}</a>
				<c:if test="${recipe.user.sex=='男'}"><img src="../../images/recipe/male.jpg"/></c:if>
				<c:if test="${recipe.user.sex=='女'}"><img src="../../images/recipe/female.jpg"/></c:if></span>
		...
		<div class="comment">
			...
			<div class="writecomment">
				<form action="" name="comment" method="post"> 
					...
					<br/>
					<input style="float:right;margin-left: 10px;margin-top: 10px;" type="image" src="../../images/recipe/commentbutton.jpg"  onclick="submitComment()"/>
				</form>
			</div>
			<div class="comments">
				...
				<c:forEach items="${comments}" var="comment">
					...
					<span><a href="/info/goUserIndex.action?user_id=${comment.user.id}"><font class="font6">${comment.user.username }</font></a></span>
				...
				<div class="pages" style="text-align: center;margin-top: 15px;float:left;width:600px;">
					<c:if test="${pageNow!=1}"><a href="/recipe/goRecipeDetail.action?recipe_id=${recipe.id}&pageTo=${pageNow-1}">上一页</a></c:if>
    				<c:forEach begin="1" end="${pageCount}" var="i" step="1">
    				<a href="/recipe/goRecipeDetail.action?recipe_id=${recipe.id}&pageTo=${i}">${i}</a>
    				</c:forEach>
    				<c:if test="${pageNow!=pageCount}"><a href="/recipe/goRecipeDetail.action?recipe_id=${recipe.id}&pageTo=${pageNow+1}">下一页</a></c:if>
