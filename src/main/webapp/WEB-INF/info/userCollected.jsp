...
<body onload="judgeHasConcerned()">
	...
	<div class="main_index">
		...
		<div class="pages">
			<c:if test="${pageNow!=1}"><a href="${pageContext.request.contextPath}/info/goUserCollected.action?user_id=${user.id}&pageTo=${pageNow-1}">上一页</a></c:if>
			<c:forEach begin="1" end="${pageCount}" var="i" step="1">
			<a href="${pageContext.request.contextPath}/info/goUserCollected.action?user_id=${user.id}&pageTo=${i}">${i}</a>
			</c:forEach>
			<c:if test="${pageNow!=pageCount}"><a href="${pageContext.request.contextPath}/info/goUserCollected.action?user_id=${user.id}&pageTo=${pageNow+1}">下一页</a></c:if>
