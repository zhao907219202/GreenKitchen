	...
	<div class="main_index">
		...
		<div class="introduction">
			...
			<div class="user_button">
				<input id="doConcern" onclick="doConcern()" type="image" src="${pageContext.request.contextPath}/images/info/concernButton.jpg" style="width:70px;height:30px;float:left;margin-left: 30px;" />
				<input id="cancelConcern" onclick="cancelConcern()" type="image" src="${pageContext.request.contextPath}/images/info/cancelconcern.jpg" style="width:70px;height:30px;float:left;margin-left: 30px;" />
			</div>
		...
		<div class="list">
			...
			<table class="list_table">
				<c:forEach items="${fans}" var="fan" varStatus="status">
					<tr style="height:35px;">
						<td style="width:50px;text-align: center;"><font class="font18">${status.count}</td>
						<td style="width:650px;"><a href="${pageContext.request.contextPath}/info/goUserIndex.action?user_id=${fan.id}">
						<font class="font16">${fan.username}</font></a></td>
						<td><font class="font18">${fan.fansnum}粉丝</font></td>
					</tr>
				</c:forEach>
			</table>
		</div>
		...
		<div class="pages">
			<c:if test="${pageNow!=1}"><a href="${pageContext.request.contextPath}/info/goFans.action?user_id=${user.id}&pageTo=${pageNow-1}">上一页</a></c:if>
			<c:forEach begin="1" end="${pageCount}" var="i" step="1">
			<a href="${pageContext.request.contextPath}/info/goFans.action?user_id=${user.id}&pageTo=${i}">${i}</a>
			</c:forEach>
			<c:if test="${pageNow!=pageCount}"><a href="${pageContext.request.contextPath}/info/goFans.action?user_id=${user.id}&pageTo=${pageNow+1}">下一页</a></c:if>
