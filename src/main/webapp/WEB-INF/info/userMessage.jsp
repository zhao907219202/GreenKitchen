...
<title>绿色厨房</title>
	...
	function submitMessage(){
		if(clicked==false){
			document.message.action="/GreenKitchen/reply/submitMessage.action";
		}
		document.message.submit();
	}
	function focusMessage(obj){
		document.message.action="/GreenKitchen/reply/submitMessageReply.action?message_id="+obj.id;
	...
	<div class="main_index">
		...
		<div class="comment">
			...
			<div class="writecomment">
				<form action="" name="message" method="post">
					...
					<br/>
					<input style="float:right;margin-left: 10px;margin-top: 10px;" type="image" src="/GreenKitchen/images/recipe/commentbutton.jpg" onclick="submitMessage()" />
				</form>
			</div>
			<div class="comments">
				...
				<c:forEach items="${messages}" var="message">
					...
					<span><a href="/GreenKitchen/info/goUserIndex.action?user_id=${message.userByWriterId.id}"><font class="font6">${message.userByWriterId.username }</font></a></span>
		...
		<c:if test="${!empty messages }">
			<div class="pages" style="margin-top: 5px;">
				<c:if test="${pageNow!=1}"><a href="/GreenKitchen/info/goUserMessage.action?user_id=${user.id}&pageTo=${pageNow-1}">上一页</a></c:if>
				<c:forEach begin="1" end="${pageCount}" var="i" step="1">
				<a href="/GreenKitchen/info/goUserMessage.action?user_id=${user.id}&pageTo=${i}">${i}</a>
				</c:forEach>
				<c:if test="${pageNow!=pageCount}"><a href="/GreenKitchen/info/goUserMessage.action?user_id=${user.id}&pageTo=${pageNow+1}">下一页</a></c:if>
