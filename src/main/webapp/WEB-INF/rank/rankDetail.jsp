...
<body>
	...
	<div class="main_detail">
			...
			<table class="list_table">
				<c:forEach items="${recipes}" var="recipe" varStatus="status">
					<tr style="height:35px;">
						...
						<td style="width:650px;"><a href="/recipe/goRecipeDetail.action?recipe_id=${recipe.id}">
						<font class="font16">${recipe.title}</font></a>
						<span style="color:gray;font-size: 12px;">by</span>
						<a href="/info/goUserIndex.action?user_id=${recipe.user.id}"><font class="font3">${recipe.user.username}</font></a></td>
		...
		<div class="pages">
			<c:if test="${pageNow!=1}"><a href="/rank/goRankDetail.action?pageTo=${pageNow-1}&title_id=${title_id}">上一页</a></c:if>
			<c:forEach begin="${(pageNow-2)>1?(pageNow-2):1}" end="${(pageNow+2)<pageCount?(pageNow+2):pageCount}" var="i" step="1">
			<a href="/rank/goRankDetail.action?pageTo=${i}&title_id=${title_id}">${i}</a>
			</c:forEach>
			<c:if test="${pageNow!=pageCount}"><a href="/rank/goRankDetail.action?pageTo=${pageNow+1}&title_id=${title_id}">下一页</a></c:if>
