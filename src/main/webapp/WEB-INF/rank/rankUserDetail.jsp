...
<body>
	...
	<div class="main_detail">
			...
			<table class="list_table">
				<c:forEach items="${users}" var="user" varStatus="status">
					<tr style="height:35px;">
						...
						<td style="width:650px;"><a href="/GreenKitchen/info/goUserIndex.action?user_id=${user.id}">
		...
		<div class="pages">
			<c:if test="${pageNow!=1}"><a href="/GreenKitchen/rank/goRankUserDetail.action?pageTo=${pageNow-1}">上一页</a></c:if>
			<c:forEach begin="${(pageNow-2)>1?(pageNow-2):1}" end="${(pageNow+2)<pageCount?(pageNow+2):pageCount}" var="i" step="1">
			<a href="/GreenKitchen/rank/goRankUserDetail.action?pageTo=${i}">${i}</a>
			</c:forEach>
			<c:if test="${pageNow!=pageCount}"><a href="/GreenKitchen/rank/goRankUserDetail.action?pageTo=${pageNow+1}">下一页</a></c:if>
